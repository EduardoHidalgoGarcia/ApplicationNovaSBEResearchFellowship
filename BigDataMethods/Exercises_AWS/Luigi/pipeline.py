...
class ConvertToParquet(luigi.Task):
    ...
    def run(self):
        ...
        command = "aws emr add-steps --cluster-id " + cluster_id + " --steps Type=spark,Name=Parquet,Args=[--deploy-mode,cluster,--master,yarn,--conf,spark.yarn.submit.waitAppCompletion=true,s3a://daniel-sharp/profeco/parquet.py],ActionOnFailure=CONTINUE"
        proc = subprocess.Popen(command, shell =True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        tmp = proc.stdout.read()
...
class Agg(luigi.Task):
    ...
    def run(self):
        ...
        command = "aws emr add-steps --cluster-id " + cluster_id + " --steps Type=spark,Name=Agg,Args=[--deploy-mode,cluster,--master,yarn,--conf,spark.yarn.submit.waitAppCompletion=true,s3a://daniel-sharp/profeco/agg.py],ActionOnFailure=CONTINUE"
        proc = subprocess.Popen(command, shell =True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        tmp = proc.stdout.read()
...
class EndPipeline(luigi.Task):
    ...
    def run(self):
        comm = 'aws s3 cp s3://daniel-sharp/profeco/agg/ ./ --recursive --exclude "*" --include "*.csv"'
        proc = subprocess.Popen(comm, shell =True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        with self.output().open('w') as f:
            f.write("Pipeline terminado")
